function start_game(){
    ...
    let user_score = parseInt(document.getElementById('user_score').value);
    let computer_score = parseInt(document.getElementById('computer_score').value);
    for(i = 0; i < user_option.length; i++){
        if(user_option[i].checked){
            ...
            if((user_option == "pedra" && computer_option == "pedra") || (user_option == "papel" && computer_option == "papel") || (user_option == "tesoura" && computer_option == "tesoura")){
                ...
                document.getElementById("computer_score").setAttribute("value", computer_score += 1); //Adicionar pontuação
                document.getElementById('computer_scoreboard').innerHTML = computer_score;
            }
                ...
                document.getElementById("user_score").setAttribute("value", user_score += 1); //Adicionar pontuação
                document.getElementById('user_scoreboard').innerHTML = user_score;
            }
        }
    }
}